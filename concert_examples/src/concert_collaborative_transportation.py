...
with suppress_stdout():
    ...
    urdf_writer = UrdfWriter(speedup=True, floating_base=True, verbose=False)
    homing_joint_map = {}
    ...
    homing_joint_map[str(data['name'])] = -2.0
    ...
    # urdf_writer.add_module('concert/module_spraying_tool_concert.json')

    urdf_writer.add_module('module_dagana.json')
    homing_joint_map['dagana_E_claw_joint'] = 1.0
    urdf_writer.add_dagana_claws('concert_formwork_claws')
...
# # content = urdf_writer.process_urdf()
# # # open(f'/tmp/modularbot.urdf', 'w').write(content)

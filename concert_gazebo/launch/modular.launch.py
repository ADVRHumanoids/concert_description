...
def generate_launch_description():
    ...
    def _is_drill_camera_name(sensor_name):
        return str(sensor_name).startswith('drill_camera')

    def _filter_camera_names_by_launch_args(names, realsense_enabled, drill_camera_enabled):
        filtered_names = []
        for name in names:
            is_drill_camera = _is_drill_camera_name(name)
            if (is_drill_camera and drill_camera_enabled) or (not is_drill_camera and realsense_enabled):
    ...
    def _build_generator_sensor_args(overrides=None):
        overrides = overrides or {}
        cmd_args = []
        for sensor_name in sensor_launch_args:
            cmd_args.extend([
                f' {sensor_name}:=',
                overrides.get(sensor_name, LaunchConfiguration(sensor_name)),
            ])
        return cmd_args
    ...
    arg_launch_arguments = [
        ...
        DeclareLaunchArgument(sensor_name, default_value=default_value)
        for sensor_name, default_value in sensor_launch_args.items()
    ] + [
    ...
    def _create_dynamic_bridge_node(context, *args, **kwargs):
        ...
        drill_camera_enabled = LaunchConfiguration('drill_camera').perform(context).strip().lower() == 'true'
        cameras_enabled = realsense_enabled or drill_camera_enabled
        ...
        if cameras_enabled:
            sensor_names_configured = isinstance(sensors_config.get('sensor_names'), dict)
            generic_camera_defaults = [] if sensor_names_configured else default_camera_names

            aligned_depth_rs_names = _filter_camera_names_by_launch_args(
                _resolve_sensor_names(sensors_config, 'camera/realsense_depth_aligned', []),
                realsense_enabled,
                drill_camera_enabled,
            )
            rs_names = _filter_camera_names_by_launch_args(
                _resolve_sensor_names(sensors_config, 'camera/realsense', []),
                realsense_enabled,
                drill_camera_enabled,
            )
            camera_names = _filter_camera_names_by_launch_args(
                _resolve_sensor_names(sensors_config, 'camera/generic', generic_camera_defaults),
                realsense_enabled,
                drill_camera_enabled,
            )
